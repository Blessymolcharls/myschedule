...
import { scheduleApi, commitmentApi, taskApi } from '../services/api';
...
export const CalendarPage = () => {
  ...
  const { activeSession, startSession } = useTimer();

  const [currentDate, setCurrentDate] = useState(new Date());
  const [viewMode, setViewMode] = useState('24h'); // '24h' | 'week'
  ...
  const handlePrev = () => {
    ...
    if (viewMode === '24h' || viewMode === 'day') d.setDate(d.getDate() - 1);
  ...
  const handleToggleLock = async (ev) => {
    try {
      await scheduleApi.updateEvent(ev._id, { isLocked: !ev.isLocked });
      fetchCalendarEvents();
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#718096] mt-1 font-medium">
            24-hour visual timelines, recurring commitments, and non-conflicting intelligent focus blocks.
          </p>
        </div>
      ...
      <div className="pastel-card p-4 bg-[#FFFFFF] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            ...
            className="btn-secondary-pastel px-3.5 py-1.5 text-xs font-bold"
          >
            Today
          </button>
      ...
      {/* Main View Area: 24h Daily Timeline OR Week Grid */}
      {viewMode === '24h' || viewMode === 'day' ? (
        <TimelineGrid24Hour
          date={currentDate}
          events={events}
          ...
          onSlotClick={() => setCommitmentModalOpen(true)}
            ...
            {weekDays.map((day, idx) => {
              ...
              return (
                <div
                  ...
                  className={`flex flex-col rounded-2xl border p-3 min-h-[420px] transition-all cursor-pointer group hover:border-[#8B7BE8] hover:shadow-card ${
                    isToday
                ...
                >
                  ...
                  <div className="pb-2 mb-2 border-b border-[#F0EDF9] text-center">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-[#718096] group-hover:text-[#8B7BE8] transition-colors">
                    ...
                    <div
                      className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold mt-1 ${
                        isToday
                          ...
                          : 'text-[#26324A] group-hover:bg-[#ECE9FB]'
                      }`}
                    >
                      {day.getDate()}
                    </div>
